...
class TemplateDefinitionBuilder {
    ...
    buildTemplateFunction(asts, variables) {
        ...
        for (const variable$$1 of variables) {
            ...
            const declaration = variable(scopedName).set(expression).toDeclStmt(INFERRED_TYPE, [
                StmtModifier.Final
            ]);
    ...
    _computeDirectivesArray(directives) {
        ...
        return {
            directivesArray: directiveExpressions.length ?
                this.constantPool.getConstLiteral(literalArr(directiveExpressions), /* forceShared */ true) :
                literal(null),
            directiveIndexMap
        };
    }
    ...
    visitElement(element) {
        ...
        const nullNode = literal(null, INFERRED_TYPE);
        ...
        this.instruction(this._creationMode, element.sourceSpan, Identifiers$1.createElement, ...parameters);
        const implicit = variable(this.contextParameter);
    ...
    _visitDirectives(directives, implicit, nodeIndex, directiveIndexMap) {
        for (let directive of directives) {
            ...
            const kind = directive.directive.isComponent ? 2 /* Component */ : 1;
    ...
    for (let dependency of type.diDeps) {
        ...
        if (token) {
            ...
            if (tokenRef === elementRef) {
                ...
                const value = token.identifier != null ? outputCtx.importExpr(tokenRef) : literal(tokenRef);
...
class ValueConverter extends AstMemoryEfficientTransformer {
    ...
    visitLiteralMap(ast, context) {
        return new BuiltinFunctionCall(ast.span, this.visitAll(ast.values), values => {
            ...
            // calls to literal factories that compose the literal and will cache intermediate
            // values. Otherwise, just return an literal array that contains the values.
            const literal$$1 = literalMap(values.map((value, index) => ({ key: ast.keys[index].key, value, quoted: ast.keys[index].quoted })));
            return values.every(a => a.isConstant()) ?
                this.outputCtx.constantPool.getConstLiteral(literal$$1, true) :
                getLiteralFactory(this.outputCtx, literal$$1);
        });
